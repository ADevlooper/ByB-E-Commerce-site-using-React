...
const cosmeticColors = [
  { name: 'Fair', code: '#FFE4C4' },
  { name: 'Light', code: '#DEB887' },
  { name: 'Medium', code: '#CD853F' },
  { name: 'Tan', code: '#D2691E' },
  { name: 'Deep', code: '#8B4513' },
  { name: 'Rich', code: '#654321' }
];

const furnitureColors = ['Black', 'White', 'Brown', 'Beige', 'Gray'];
const furnitureModels = ['Classic', 'Modern', 'Contemporary', 'Vintage'];
const fragranceTypes = ['Fresh', 'Woody', 'Floral', 'Citrus', 'Oriental'];
...
function ProductDetail() {
  ...
  const isCosmetic = (category) => ['skincare', 'fragrances'].includes(category?.toLowerCase());
  const isPerfume = (category) => category?.toLowerCase() === 'fragrances';
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        setProduct(data);
        setCurrentImageIndex(0); // Reset to first image when new product loads
      } catch (err) {
  ...
  useEffect(() => {
    const fetchRelatedProducts = async () => {
      ...
      try {
        ...
        const data = await response.json();
        // Filter out the current product and limit to 10 products
        const filtered = data.products.filter(p => p.id !== product.id).slice(0, 10);
  ...
  const handleBuyNow = () => {
    ...
    const productWithOptions = {
      ...
      ...(isFurniture(product.category) && { selectedColor, selectedModel }),
      ...(isCosmetic(product.category) && !isPerfume(product.category) && { selectedColor }),
  ...
  if (error || !product) {
    return (
      <div className="container mx-auto px-8 py-8 text-center">
        ...
        <Link to="/products" className="text-red-800 hover:text-red-900">
          ← Back to Products
        </Link>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-8 py-8">
      {/* Back button */}
      <Link to="/products" className="inline-flex items-center gap-2 px-4 py-2 bg-red-800 text-white rounded-md hover:bg-red-900 transition-all duration-300 mb-8 group">
              ...
              {product.images.map((image, index) => (
                <button
                  ...
                  className={`flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden border-2 transition-all duration-200 ${
                    index === currentImageIndex ? 'border-red-800' : 'border-gray-200 hover:border-red-400'
                  }`}
                >
                  <img
                    src={image}
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-baseline justify-between">
              ...
              <div className="flex items-center gap-1">
                {Array.from({ length: 5 }, (_, i) => (
                  <svg
                    key={i}
                    className={`w-4 h-4 ${i < Math.floor(product.rating) ? 'text-yellow-400 fill-current' : 'text-white'}`}
                    viewBox="0 0 24 24"
                  >
          ...
          {product.warrantyInformation && (
            <div className="flex items-center w-fit gap-2 bg-gradient-to-r from-green-400 to-green-600 text-white pl-3 pr-[25%] py-2 rounded-2xl">
              ...
              <span className="font-bold">{product.warrantyInformation}</span>
            </div>
          )}
          ...
          <div className="flex gap-4">
            <button
              ...
              className="flex-1 border-2 border-red-800 text-red-800 px-6 py-3 rounded-md hover:bg-red-800 hover:text-white transition-colors flex items-center justify-center gap-2"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Product Options */}
      {(isFurniture(product.category) || isCosmetic(product.category) || isPerfume(product.category)) && (
            ...
            {(isFurniture(product.category) || (isCosmetic(product.category) && !isPerfume(product.category))) && (
              <div className="space-y-4">
                <h3 className="font-semibold text-lg">
                  {isCosmetic(product.category) ? 'Select Shade' : 'Select Color'}
                </h3>
                <div className="flex flex-wrap gap-3">
                  {(isCosmetic(product.category) ? cosmeticColors : furnitureColors).map(color => (
                    <button
                      key={typeof color === 'string' ? color : color.name}
                      onClick={() => setSelectedColor(typeof color === 'string' ? color : color.name)}
                      className={`flex flex-col items-center gap-2 p-2 rounded-md transition-all ${
              ...
              <div key={relatedProduct.id} className="flex-shrink-0 w-64 bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                <Link to={`/product/${relatedProduct.id}`} className="block relative">
                  <div className="h-48 bg-gray-200 flex items-center justify-center">
                    ...
                    {/* Offer Tag */}
                    {relatedProduct.discountPercentage >= 5 && (
                      <span className="absolute top-2 left-2 bg-red-100 text-red-800 text-sm font-medium px-3 py-1 rounded">
                        {Math.floor(relatedProduct.discountPercentage)}% OFF
                      </span>
                    )}
                  </div>
                  <div className="p-4">
                    ...
                    <div className="flex items-baseline gap-2">
                      <span className="text-lg font-bold text-red-800">${relatedProduct.price}</span>
                      <span className="text-sm text-gray-500 line-through">
                        ${Math.round(relatedProduct.price * (1 + Math.floor(relatedProduct.discountPercentage) / 100))}
                      </span>
                    </div>
                  </div>
                </Link>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-green-600">{Math.floor(product.discountPercentage)}% off</span>
            </div>
            <div className="flex items-center gap-4">
            ...
            product.reviews.map((review, index) => (
              <div key={index} className="border-b border-gray-200 pb-6 last:border-b-0">
                ...
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center gap-2">
                    <button className="border border-red-800 text-red-800 px-3 py-1 rounded-md hover:bg-red-800 hover:text-white transition-colors">
                      Helpful
                    </button>
                    <button className="bg-red-800 text-white px-3 py-1 rounded-md hover:bg-red-900 transition-colors flex items-center gap-1">
